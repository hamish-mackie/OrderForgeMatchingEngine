...
public:
    ...
    void forward_read_pointer(uint64_t new_read_offset) {
        read_ptr_offset_ += new_read_offset;
    }
...
private:
    ...
    std::pair<std::atomic<uint64_t>, std::atomic<size_t>> get_current_write() {
        return {write_ptr_offset_.load(std::memory_order_relaxed),
                write_block_index_.load(std::memory_order_relaxed)};
    }
